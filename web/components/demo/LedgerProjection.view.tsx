...
import { API_BASE, formatMoney, type ContractView, type Role } from "@/lib/api";
import { Cid } from "./ui";
...
export type ProjectionViewMode = "cards" | "raw";
...
export function ProjectionView({
  ...
  role: Role;
  view: ProjectionViewMode;
  rows: ContractView[];
  loading: boolean;
  error: string | null;
  roundId: string | null;
  onRole?: (r: Role) => void;
  onView?: (v: ProjectionViewMode) => void;
}) {
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line px-5 py-3.5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex overflow-hidden rounded-btn border border-line">
            {(["cards", "raw"] as ProjectionViewMode[]).map((v) => (
              <button
                key={v}
                type="button"
                onClick={() => onView?.(v)}
